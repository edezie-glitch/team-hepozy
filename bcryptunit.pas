...
{ ============================================================
  bcryptunit.pas — pure-Pascal, cross-platform bcrypt
  (Windows/Linux/macOS — no dependency on Unix crypt())

  Ported line-for-line from passlib's pure-Python reference
  implementation (passlib/crypto/_blowfish/{__init__,base}.py),
  which is itself derived from jBCrypt. The Blowfish P-array and
  S-box constants below were extracted programmatically from
  that same source — not hand-transcribed — to eliminate copy
  error risk in ~1,060 hex constants.

  Implements: EKS-Blowfish key schedule (salt-mixed setup +
  repeated expansion), the bcrypt "OrpheanBeholderScryDoubt"
  64-round encryption of the magic constant, and bcrypt's own
  base64 variant (alphabet: ./A-Za-z0-9, big-endian bit packing).

  Tested against real hashes produced by Python's `bcrypt`
  library (same algorithm passlib's C backend uses) — see
  test_bcrypt_pascal.pas.
  ============================================================ }
...
// verifies PlainPassword against a full "$2b$12$..." (or $2a$/$2y$) hash
function BcryptVerify(const PlainPassword, StoredHash: string): Boolean;
// produces a new "$2b$<cost>$..." hash for a plaintext password
function BcryptHash(const PlainPassword: string; Cost: Integer = 12): string;
// exposed for isolated debug testing only
function Bcrypt64EncodeBytes(const Data: array of Byte; DataLen: Integer): string;
function Bcrypt64DecodeBytes(const S: string): TBytes;
function RawBcrypt(const Password: string; const SaltB64: string; LogRounds: Integer): string;
// debug-only: run Encipher with unmixed default P/S boxes on (0,0)
procedure DebugEncipherDefault(out L, R: LongWord);

implementation

const
  P_INIT: array[0..17] of LongWord = (
  ...
  S_INIT: array[0..3, 0..255] of LongWord = (
    (
  ...
  BCRYPT_CDATA: array[0..5] of LongWord = (
...
{ ---- Feistel cipher core, ported from BlowfishEngine.encipher ---- }
procedure Encipher(var State: TBFState; var L, R: LongWord);
var
  i: Integer;
  Tmp, NewL, NewR: LongWord;
begin
  L := L xor State.P[0];
  i := 1;
  while i < 17 do
  begin
    Tmp := ((State.S[0][(L shr 24) and $FF] + State.S[1][(L shr 16) and $FF])
            xor State.S[2][(L shr 8) and $FF]) + State.S[3][L and $FF];
    R := Tmp xor State.P[i] xor R;
    // swap L, R (matches Python's `l, r = r, l`)
    Tmp := L; L := R; R := Tmp;
    Inc(i);
  end;
  // NOTE: P[16] was already consumed above as P[i] when i=16 (the last
  // loop iteration) — it is NOT a separate final-whitening step, and
  // there is no additional swap here: 16 in-loop swaps is an even
  // count, so L/R are already in the correct orientation. Matches
  // Python's `return r ^ P[17], l` exactly — only P[17] applies here,
  // and the two variables are NOT swapped again before returning.
  NewL := R xor State.P[17];
  NewR := L;
  L := NewL;
  R := NewR;
end;

{ ---- key/salt -> 18 x 32-bit words, repeating/truncating as needed,
       ported from BlowfishEngine.key_to_words ---- }
procedure KeyToWords(const Data: string; out Words: array of LongWord; WordCount: Integer);
var
  Buf: array of Byte;
  DLen, TotalBytes, i, SrcPos: Integer;
begin
  DLen := Length(Data);
  TotalBytes := WordCount * 4;
  SetLength(Buf, TotalBytes);

  if DLen = 0 then
  begin
    FillChar(Buf[0], TotalBytes, 0);
  end
  else
  begin
    SrcPos := 0;
    ...
    begin
      Buf[i] := Byte(Data[(SrcPos mod DLen) + 1]);
      Inc(SrcPos);
    end;
  end;

  for i := 0 to WordCount - 1 do
    Words[i] := (LongWord(Buf[i*4]) shl 24) or (LongWord(Buf[i*4+1]) shl 16) or
                (LongWord(Buf[i*4+2]) shl 8) or LongWord(Buf[i*4+3]);
end;

{ ---- standard Blowfish key schedule expand, ported from
       BlowfishEngine.expand ---- }
procedure Expand(var State: TBFState; const KeyWords: array of LongWord; KeyWordCount: Integer);
var
  i, j: Integer;
  L, R: LongWord;
begin
  for i := 0 to 17 do
    State.P[i] := State.P[i] xor KeyWords[i mod KeyWordCount];
...
{ ---- EKS salted key schedule setup, ported from
       BlowfishEngine.eks_salted_expand ---- }
procedure EksSaltedExpand(var State: TBFState; const KeyWords: array of LongWord; KeyWordCount: Integer;
  const SaltWords: array of LongWord; SaltWordCount: Integer);
var
  i, j, s: Integer;
  L, R: LongWord;
begin
  ...
  begin
    L := L xor SaltWords[s];
    R := R xor SaltWords[s+1];
    Inc(s, 2);
    if s = SaltWordCount then s := 0;
    Encipher(State, L, R);
    State.P[i] := L;
    State.P[i+1] := R;
    Inc(i, 2);
...
{ ---- bcrypt's own base64 variant: alphabet "./A-Za-z0-9",
       big-endian (standard RFC4648) bit packing.
       Ported from Base64Engine._encode_bytes_big /
       _decode_bytes_big. ---- }

function Bcrypt64EncodeBytes(const Data: array of Byte; DataLen: Integer): string;
var
  Chunks, Tail, Idx, i: Integer;
  V1, V2, V3: Byte;
  Codes: array of Byte;
  CodeCount: Integer;
begin
  Chunks := DataLen div 3;
  Tail := DataLen mod 3;
  SetLength(Codes, ((DataLen + 2) div 3) * 4);
  CodeCount := 0;
  Idx := 0;

  for i := 1 to Chunks do
  begin
    V1 := Data[Idx]; V2 := Data[Idx+1]; V3 := Data[Idx+2];
    Codes[CodeCount] := V1 shr 2; Inc(CodeCount);
    Codes[CodeCount] := ((V1 and $03) shl 4) or (V2 shr 4); Inc(CodeCount);
    Codes[CodeCount] := ((V2 and $0F) shl 2) or (V3 shr 6); Inc(CodeCount);
    Codes[CodeCount] := V3 and $3F; Inc(CodeCount);
    Inc(Idx, 3);
  end;
  ...
  begin
    V1 := Data[Idx];
    Codes[CodeCount] := V1 shr 2; Inc(CodeCount);
    Codes[CodeCount] := (V1 and $03) shl 4; Inc(CodeCount);
  end
  else if Tail = 2 then
  begin
    V1 := Data[Idx]; V2 := Data[Idx+1];
    Codes[CodeCount] := V1 shr 2; Inc(CodeCount);
    Codes[CodeCount] := ((V1 and $03) shl 4) or (V2 shr 4); Inc(CodeCount);
    Codes[CodeCount] := (V2 and $0F) shl 2; Inc(CodeCount);
  end;

  SetLength(Result, CodeCount);
  for i := 0 to CodeCount - 1 do
    Result[i+1] := BCRYPT_ALPHABET[Codes[i] + 1];
end;

function Bcrypt64CharIndex(C: Char): Integer;
begin
...
begin
  ...
  Chunks := InLen div 4;
  Tail := InLen mod 4;
  SetLength(OutBytes, (InLen div 4) * 3 + 3);
  OutCount := 0;
  Idx := 1;

  for i := 1 to Chunks do
  begin
    V1 := Bcrypt64CharIndex(S[Idx]);
    V2 := Bcrypt64CharIndex(S[Idx+1]);
    V3 := Bcrypt64CharIndex(S[Idx+2]);
    V4 := Bcrypt64CharIndex(S[Idx+3]);
    OutBytes[OutCount] := Byte((V1 shl 2) or (V2 shr 4)); Inc(OutCount);
    OutBytes[OutCount] := Byte(((V2 and $F) shl 4) or (V3 shr 2)); Inc(OutCount);
    OutBytes[OutCount] := Byte(((V3 and $3) shl 6) or V4); Inc(OutCount);
    Inc(Idx, 4);
  end;
  ...
  SetLength(OutBytes, OutCount);
  Result := OutBytes;
end;

{ ---- core bcrypt function, ported from raw_bcrypt() ---- }
function RawBcrypt(const Password: string; const SaltB64: string; LogRounds: Integer): string;
var
  State: TBFState;
  PassBytes: string;
  PassWords: array[0..17] of LongWord;
  SaltWords18: array[0..17] of LongWord;
  SaltWords4: array[0..3] of LongWord;
  SaltRaw: TBytes;
  Rounds, i, RoundIdx: Integer;
  Data: array[0..5] of LongWord;
  L, R: LongWord;
  DigestBytes: array[0..23] of Byte;
  SaltPadded: string;
  j: Integer;
begin
  // password: append NUL, matches add_null_padding for 2a/2y/2b idents
  PassBytes := Password + #0;

  // decode salt from bcrypt64, truncate/require 16 raw bytes
  SaltRaw := Bcrypt64DecodeBytes(SaltB64);
  if Length(SaltRaw) < 16 then
    raise Exception.Create('bcrypt: salt too short');

  SetLength(SaltPadded, 16);
  for i := 0 to 15 do
    SaltPadded[i+1] := Chr(SaltRaw[i]);

  KeyToWords(PassBytes, PassWords, 18);
  KeyToWords(SaltPadded, SaltWords18, 18);
  for i := 0 to 3 do
    SaltWords4[i] := SaltWords18[i];

  FillChar(State, SizeOf(State), 0);
  Move(P_INIT, State.P, SizeOf(P_INIT));
  Move(S_INIT, State.S, SizeOf(S_INIT));

  EksSaltedExpand(State, PassWords, 18, SaltWords4, 4);

  Rounds := 1 shl LogRounds;
  for RoundIdx := 1 to Rounds do
  begin
    Expand(State, PassWords, 18);
    Expand(State, SaltWords18, 18);
  end;

  for i := 0 to 5 do
    Data[i] := BCRYPT_CDATA[i];

  i := 0;
  while i < 6 do
  begin
    L := Data[i]; R := Data[i+1];
    for RoundIdx := 1 to 64 do
      Encipher(State, L, R);
    Data[i] := L; Data[i+1] := R;
    Inc(i, 2);
  end;

  // pack 6 x 32-bit big-endian, then drop the last byte (23 of 24)
  for i := 0 to 5 do
  begin
    DigestBytes[i*4]   := Byte(Data[i] shr 24);
    DigestBytes[i*4+1] := Byte(Data[i] shr 16);
    DigestBytes[i*4+2] := Byte(Data[i] shr 8);
    DigestBytes[i*4+3] := Byte(Data[i]);
  end;

  Result := Bcrypt64EncodeBytes(DigestBytes, 23);
end;
...
var
  Parts: TStringArray;
  Ident, RoundsStr, SaltAndHash, Salt, ExpectedChk: string;
  LogRounds: Integer;
  Computed: string;
...
begin
  Result := False;
  // format: $2b$12$<22-char-salt><31-char-checksum>
  if (Length(StoredHash) < 4) or (StoredHash[1] <> '$') then Exit;

  DollarPos := PosEx('$', StoredHash, 2);
  if DollarPos = 0 then Exit;
  Ident := Copy(StoredHash, 2, DollarPos - 2); // e.g. '2b', '2a', '2y'

  RestAfterIdent := Copy(StoredHash, DollarPos + 1, Length(StoredHash));
  DollarPos := Pos('$', RestAfterIdent);
  if DollarPos = 0 then Exit;
  RoundsStr := Copy(RestAfterIdent, 1, DollarPos - 1);
  LogRounds := StrToIntDef(RoundsStr, -1);
  if LogRounds < 4 then Exit;

  SaltAndHash := Copy(RestAfterIdent, DollarPos + 1, Length(RestAfterIdent));
  if Length(SaltAndHash) < 22 then Exit;
  Salt := Copy(SaltAndHash, 1, 22);
  ExpectedChk := Copy(SaltAndHash, 23, Length(SaltAndHash));

  Computed := RawBcrypt(PlainPassword, Salt, LogRounds);
  Result := Computed = ExpectedChk;
end;

function BcryptHash(const PlainPassword: string; Cost: Integer): string;var
  SaltBytes: array[0..15] of Byte;
  i: Integer;
  SaltB64, Checksum: string;
begin
  // NOTE: uses a weak, non-cryptographic salt source (Pascal's Random).
  // This is fine for the test harness that validates the algorithm
  // against known hashes, but MUST be replaced with a cryptographically
  // secure random source (e.g. Windows CryptGenRandom / getrandom())
  // before this is used to hash real user passwords in production.
  Randomize;
  for i := 0 to 15 do
    SaltBytes[i] := Random(256);

  SaltB64 := Bcrypt64EncodeBytes(SaltBytes, 16);
  SaltB64 := Copy(SaltB64, 1, 22); // 16 bytes -> 22 b64 chars (2 tail bits dropped)

  Checksum := RawBcrypt(PlainPassword, SaltB64, Cost);
  Result := '$2b$' + Format('%.2d', [Cost]) + '$' + SaltB64 + Checksum;
end;
