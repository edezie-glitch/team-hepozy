{ ============================================================
  ...
  test_hmac, test_jwt, test_bcrypt_pascal), supabaseunit, bcryptunit
  (cross-platform pure-Pascal bcrypt — no OS-specific crypt() call,
...
uses
  ...
  fphttpclient, opensslsockets, bcryptunit,
...
// bcrypt verify via pure-Pascal bcryptunit — proven cross-platform
// and cross-compatible with passlib's bcrypt hashes (see
// test_bcrypt_pascal.pas / test_bcrypt_battery.pas, tested against
// real hashes produced by Python's bcrypt library)
function VerifyPassword(const PlainPassword, StoredHash: string): Boolean;
begin
  Result := BcryptVerify(PlainPassword, StoredHash);
end;

function HashPassword(const PlainPassword: string): string;
begin
  // NOTE: BcryptHash's internal salt generation uses Pascal's Random(),
  // seeded via Randomize — NOT cryptographically secure. This is fine
  // for the test harness but MUST be replaced with a CSPRNG (Windows
  // CryptGenRandom / BCryptGenRandom, or Linux getrandom()) before
  // this hashes real user passwords in production. Flagged here
  // explicitly, not silently shipped as if it were production-ready.
  Result := BcryptHash(PlainPassword, 12);
end;
